...
//  FlappyDragon
//
//  Created by Gabriel on 31/07/21.
//
...
class GameScene: SKScene {
    ...
    func addFloor() {
        ...
        floor.zPosition = 2
        floor.position = CGPoint(x: floor.size.width/2, y: size.height - gameArea - floor.size.height/2.5)
        addChild(floor)
    ...
    func addScore() {
        ...
        scoreLabel.position = CGPoint(x: size.width/2, y: size.height - 125)
    ...
    func spawnEnemies() {
        ...
        let laser = SKNode()
        laser.scene?.backgroundColor = .red
